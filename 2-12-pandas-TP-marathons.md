...
```{code-cell} ipython3
# votre code
df = pd.read_csv('data/marathon.txt', sep = '\t')
df.columns = names
df
```
...
```{code-cell} ipython3
# à vous
s1_paris_2000 = df.loc[df['year'] >= 2000, 'year'].drop_duplicates().sort_values(ascending = False)
s1_paris_2000
```
...
```{code-cell} ipython3
# à vous
df_paris_2000_ys = df.loc[df['year'] >= 2000, ['year', 'seconds']].copy()
df_paris_2000_ys = df_paris_2000_ys.drop_duplicates(subset=['year'])
...
(isinstance(df_paris_2000_ys, pd.DataFrame)
 ...
 and df_paris_2000_ys.iloc[-3].seconds == 7780)
```
...
```{code-cell} ipython3
# votre code
duration = df['duration'].dt.total_seconds()
